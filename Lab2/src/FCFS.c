...
int  FCFS(task tasks[], int nbOfTasks, int currentTime) {
  ...
  int firstArrivalTime = latest_job;
  int taskIndexToSchedule = -1;
  for (i = 0; i < nbOfTasks; i++) {
    if (tasks[i].state == RUNNABLE) {
      if (tasks[i].arrivalDate <= firstArrivalTime) {
	if (tasks[i].computationsDone < tasks[i].totalOfComputations) {
	  taskIndexToSchedule = i;
	  firstArrivalTime = tasks[i].arrivalDate;
	}
      }
    }
  }
...
int main(int argc, char *argv[]){
  ...
  while (fgets(line, sizeof line, file) != NULL ) {
    ...
    if (tasks[nbOfTasks].arrivalDate > latest_job)
      latest_job = tasks[nbOfTasks].arrivalDate;
    nbOfTasks ++;
  }
